...
describe('API Endpoints', () => {
  ...
  test('POST /fetch should handle case-insensitive Yale replacements', async () => {
    const mixedCaseHtml = '<html><head><title>YALE and Yale and yale</title></head><body><p>YALE University, Yale College, yale school</p></body></html>';
    
    nock('https://mixed-case.com')
      .get('/')
      .reply(200, mixedCaseHtml);

    const response = await request(app)
      .post('/fetch')
      .send({ url: 'https://mixed-case.com/' });
